// --- GLOBALS ---
// We access React globals directly from the window object because we are not using a bundler.
...
// 2. Hero
const Hero: React.FC<{ data: CVData['personalInfo'] }> = ({ data }) => {
  return (
...
// 3. Experience
const Experience: React.FC<{ items: ExperienceItem[]; title: string }> = ({ items, title }) => {
  const cardColors = [
...
// 5. Portfolio
const Portfolio: React.FC<{ projects: ProjectItem[] }> = ({ projects }) => {
...
const Education: React.FC<{ items: EducationItem[] }> = ({ items }) => {
  // Only render if there are items, though header usually looks good even if empty
